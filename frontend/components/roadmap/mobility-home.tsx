...
function DestinationSearchScreen({ value, origin, onChange, onCancel }: { value: string; origin: Coordinates | null; onChange: (value: string) => void; onCancel: () => void }) {
  ...
  useEffect(() => {
    if (value.trim().length < 2) {
      const reset = window.setTimeout(() => { setResults([]); setState("idle"); }, 0);
      return () => window.clearTimeout(reset);
    }
    const timer = window.setTimeout(() => {
      setState("loading");
      void geocodingProvider.search(value, origin || undefined).then((response) => {
  ...
  return <main className="roadmap-search-screen">
    <div className="search-top"><label><Search size={17} /><input autoFocus value={value} onChange={(event) => onChange(event.target.value)} placeholder="Para onde voce quer ir?" />{value && <button onClick={() => onChange("")} aria-label="Limpar"><X size={16} /></button>}</label><button onClick={onCancel}>Cancelar</button></div>
    ...
    <div className="destination-results">
      ...
      {state === "idle" && popularPlaces.map((place) => <button type="button" key={place} onClick={() => onChange(place)}><span><MapPin size={17} /></span><span><strong>{place}</strong><small>Belo Horizonte - MG</small></span><ChevronRight size={16} /></button>)}
    </div>
