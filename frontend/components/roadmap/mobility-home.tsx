...
function HomeCard({ ready, nearby, recent, activeJourney, savedJourney, onSearch, onLocation }: { ready: boolean; nearby: Stop[]; recent: ReturnType<typeof useRecentSelections>["items"]; activeJourney: StoredJourney | null; savedJourney: StoredJourney | null; onSearch: () => void; onLocation: () => void }) {
  ...
  return <section className={`roadmap-home-card ${ready ? "ready" : ""}`}>
    ...
    <div className="roadmap-recent-list">{recent.slice(0, 3).map((item) => <Link href={selectionHref(item)} key={`${item.kind}-${item.id}`}><Clock3 size={16} /><span><strong>{item.label}</strong><small>{item.description}</small></span></Link>)}{recent.length === 0 && popularPlaces.slice(0, 3).map((label) => <button key={label} onClick={onSearch}><Clock3 size={16} /><span><strong>{label}</strong></span></button>)}</div>
  </section>;
}
